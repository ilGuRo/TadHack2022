...
import { ClickToCallConfiguration } from '../api/ClickToCallConfiguration';
...
function Call() {
  ...
  function connectToEngageDigital() {
    const engageDomain = ClickToCallConfiguration.domain;
    ...
    if (isEngageDigitalSdkLoaded) {
      ...
      const config = {
        log: {
          console: { enable: ClickToCallConfiguration.consoleLog },
        },
  ...
  function makeCall() {
    const callToNum = ClickToCallConfiguration.callToNum;
    try {
      if (engageDigital) {
        engageDigital.makeCall(callToNum, {
          mediaConstraints: {
            audio: true,
            video: ClickToCallConfiguration.callType === 'video' ? true : false,
          },
          joinWithVideoMuted: ClickToCallConfiguration.joinWithVideoMuted,
        });
      }
    } catch (error) {
